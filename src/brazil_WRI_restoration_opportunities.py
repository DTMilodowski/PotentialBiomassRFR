...
mb2008 = useful.load_mapbiomas('BRA',timestep=23,aggregate=1)

# Load mapbiomas data for latest year (2017)
mb2017 = useful.load_mapbiomas('BRA',timestep=-1,aggregate=1)

lc_labels = ['Natural Forest','Natural Non-Forest','Plantation','Pasture','Agriculture','Urban','Other']
...
# Create pandas data frame for ease of plotting with seaborn
# - column variables
#   biome, landcover class (2008), area_ha, AGBobs, AGBpot, AGBseq
biome = []
landcover_class = []
area = []
agbobs = []
...
Not applicable for forested areas, since there is no information on regrowth/
degradation impact on biomass in forested areas. For non-forest land cover
